 ...
 */
export function detectNew(store, items, now, { answered = null, scope = null } = {}) {
  ...
  for (const source of sources) {
    ...
    // Each streamed search needs its own baseline and ID watermark. Otherwise
    // a fast combination's high IDs can hide new ads from a slower combination.
    ...
    if (!ready) {
      ...
      for (const item of list) {
        ...
        if (!seen[item.key] && item.numericId && item.numericId > maximumId) fresh.push(item);
      }
    } else {
    ...
    baselines[baselineKey] = true;
  }

  return { fresh, baselineItems, seen: pruneSeen(seen), kaMaxId, baselines, watermarks };
}
