...
export function recoveryIntervalMs(source) {
  const state = recovery.get(source);
  return state && state.until > Date.now() ? state.intervalMs : 0;
}
...
export function pauseSource(source, result = {}) {
  ...
  recovery.set(source, { until: Date.now() + 10 * 60_000,
    intervalMs: Math.min(30_000, Math.max(10_000, recoveryIntervalMs(source) * 2)) });
