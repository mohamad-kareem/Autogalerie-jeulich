...
async function fetchPage(url, label) {
  return request(url, { label, timeoutMs: 9_000, retries: 0 });
}
...
export async function searchKleinanzeigen(f) {
  let placeId = null;
  if (f.zip) {
  ...
  const results = await Promise.all(urls.map((url) => fetchPage(url, "Kleinanzeigen Neu")));
  ...
  for (const response of results) {
    ...
    const parsed = parseKleinanzeigenPage(response.body);
