...
export function autoscoutUrl(f, page = 1) {
...
export function parseAutoScoutPage(html) {
  ...
  let pageData;
  try {
    pageData = JSON.parse(match[1])?.props?.pageProps;
  ...
  for (const entry of listings) {
    ...
    if (entry.location?.countryCode && entry.location.countryCode !== "DE") continue;
    // Relaxed-filter recommendations are not matches for the chosen search.
    if (/recommend|relax|similar|related/i.test(entry.searchResultSection || "")) continue;
    ...
    items.push({
      ...
      promoted: /sponsored|premium|topspot|boost|advert/i.test(entry.searchResultType || ""),
    });
  }

  return {
    ok: true, items, error: null,
    total: Number.isFinite(pageData.numberOfResults) ? pageData.numberOfResults : null,
    totalPages: Number.isFinite(pageData.numberOfPages) ? pageData.numberOfPages : null,
  };
}
 ...
 */
export function kleinanzeigenUrls(f, placeId, page = 1) {
...
export function parseKleinanzeigenPage(html, now = new Date()) {
  ...
  const articles = [...text.matchAll(/<article\b[^>]*data-adid=["']\d+["'][\s\S]*?<\/article>/gi)];
  // Current Astro pages expose the authoritative TOP flag in tracker props.
  // The visible badge is an SVG, so stripping tags no longer yields "TOP".
  const promotedIds = new Set();
  for (const island of text.matchAll(/<astro-island\b[^>]*>/gi)) {
    if (!island[0].includes("ImpressionTracker")) continue;
    const props = island[0].match(/\bprops="([^"]*)"/);
    if (!props) continue;
    try {
      const json = JSON.parse(props[1].replace(/&quot;/g, '"').replace(/&#39;|&#x27;/g, "'").replace(/&amp;/g, "&"));
      for (const entry of json.resultAds?.[1] || []) {
        const ad = entry?.[1]?.organicAdPreview?.[1];
        if (ad?.topAd?.[1] === true) promotedIds.add(String(ad.id?.[1]));
      }
    } catch { /* The visible badge remains available while the page streams. */ }
  }
  ...
  for (const match of articles) {
    ...
    const preceding = text.slice(Math.max(0, match.index - 2000), match.index);
    const cardStart = preceding.lastIndexOf("<li ");
    const badge = cardStart < 0 ? "" : preceding.slice(cardStart);
    // The observed TOP badge is 33x16 and anchored above its article. This
    // handles early chunks, before the authoritative tracker props arrive.
    const topBadge = /<svg\b(?=[^>]*\bwidth=["']33["'])(?=[^>]*\bheight=["']16["'])(?=[^>]*\bclass=["'][^"']*absolute[^"']*right-none[^"']*top-none)[^>]*>/i.test(badge);
    ...
    items.push({
      ...
      promoted: promotedIds.has(id) || topBadge || /(^|\s)TOP(\s|$)/.test(plain.slice(0, 60)),
    });
  }
  ...
  return {
    ok: true,
    items,
    hasMore: /(?:title|aria-label)=["']Nächste["']/i.test(text),
    error: articles.length ? null : /captcha|zugriff verweigert|access denied/i.test(text)
      ? "Kleinanzeigen hat die Anfrage blockiert."
      : /keine (?:anzeigen|ergebnisse|treffer)|0 (?:ergebnisse|anzeigen)|keine passenden/i.test(text)
        ? null : "Kleinanzeigen: Ergebnisse konnten nicht gelesen werden.",
  };
}

/* ============================================================ run a search */

async function fetchPage(url, label, onChunk) {
  return request(url, { label, timeoutMs: 8_000, retries: 0, onChunk });
}

export async function searchAutoScout(f, onBatch = () => {}, { page = 1 } = {}) {
  const url = autoscoutUrl(f, page);
  let buffer = "";
  let published = false;
  const response = await fetchPage(url, "AutoScout24 Neu", (chunk) => {
    if (published) return;
    buffer += chunk;
    if (!buffer.includes("__NEXT_DATA__") || !buffer.includes("</script>")) return;
    const parsed = parseAutoScoutPage(buffer);
    if (!parsed.ok) return;
    published = true;
    buffer = "";
    onBatch({ ok: true, items: parsed.items });
    // __NEXT_DATA__ contains the full result and pagination metadata. Nothing
    // after this script is needed; release the request for the next live check.
    return false;
  });
  if (!response.ok) return { ok: false, items: [], error: response.error, url, blocked: response.blocked,
    status: response.status, retryAfterMs: response.retryAfterMs || 0 };
  const parsed = parseAutoScoutPage(response.body);
  return { ...parsed, url, page, hasMore: parsed.totalPages !== null && parsed.totalPages !== undefined
    ? page < parsed.totalPages : parsed.items.length >= 20 };
}

export async function searchKleinanzeigen(f, onBatch = () => {}, { page = 1 } = {}) {
  let placeId = null;
  if (f.zip) {
  ...
  const urls = kleinanzeigenUrls(f, placeId, page);
  const scopes = kleinanzeigenUrls(f, placeId);
  ...
  const results = await Promise.all(urls.map(async (url, index) => {
    let buffer = "";
    const response = await fetchPage(url, "Kleinanzeigen Neu", (chunk) => {
      buffer += chunk;
      // Parse complete cards immediately; the remaining page cannot delay them.
      let end;
      while ((end = buffer.indexOf("</article>")) >= 0) {
        const fragment = buffer.slice(0, end + 10);
        buffer = buffer.slice(end + 10);
        const parsed = parseKleinanzeigenPage(fragment);
        if (parsed.items.length) onBatch({ scope: scopes[index], ok: true, partial: true, items: parsed.items });
      }
  ...
  return {
    ...
    retryAfterMs: Math.max(0, ...results.map((response) => response.retryAfterMs || 0)),
    page,
    hasMore: results.some((response) => response.parsed?.hasMore),
  };
}

export async function searchMobile(f, onBatch = () => {}, { page = 1 } = {}) {
  if (!mobileConfigured()) {
    return { ok: false, items: [], error: "mobile.de-Zugang nicht eingerichtet.", url: null, skipped: true };
  }
  return mobileSearchNewest(f, { page });
}
