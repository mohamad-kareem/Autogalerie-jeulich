...
export function describeFilters(filters) {
  ...
  return [
    ...
    f.hideDamaged ? "ohne Unfall-/Defektfahrzeuge" : "inkl. Unfall-/Defektfahrzeuge",
...
/**
 * How many searches one Kleinanzeigen check needs for this filter. The portal
 * takes one fuel and one body type per search (see kleinanzeigenUrls), so
 * each extra choice multiplies the requests — at most four.
 */
export function kleinanzeigenSearchCount(f) {
  let fuels = Math.max(1, (f.fuels || []).length);
  let bodies = Math.max(1, (f.bodies || []).length);
  while (fuels * bodies > 4) {
    if (bodies >= fuels) bodies = 1;
    else fuels = 1;
  }
  return fuels * bodies;
}
