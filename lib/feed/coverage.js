...
/** Bursts may accelerate catch-up only when the portal is not recovering. */
export function scheduleCoverage(state, status, now = Date.now()) {
  const interval = status?.recoveryIntervalMs || 0;
  let dueAt = state.dueAt;
  if (interval > 0) {
    if (state.recoveryIntervalMs !== interval) dueAt = Math.max(dueAt, now + interval * 4);
  } else if (status?.ok && status.hasMore && !status.overlap) dueAt = 0;
  return { ...state, dueAt, recoveryIntervalMs: interval };
}
