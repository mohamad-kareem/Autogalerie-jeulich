...
const PROFILES = {
  ...
  VAG_CLASSIC: { drive: BELT, km: 90_000, years: 5, cost: [400, 750] },
...
function engineRule(target, text, litres) {
  ...
  if (MAKE_GROUPS.VAG.test(make)) {
    ...
    if (petrol && l) {
      ...
      // The classic naturally aspirated and early turbo petrols (Golf 3–5,
      // Polo, Octavia, A3 …: 1.4/1.6 16V, 1.6 8V, 1.8T, 2.0 8V) all run a belt.
      if (year && year <= 2012 && [1.4, 1.6, 1.8, 2.0].includes(l) && !/tsi|tfsi|fsi/.test(text)) {
...
export function timingBeltStatus(target) {
  ...
  if (drive === "UNKNOWN") {
    ...
    return {
      ...
      detail: wouldBeDue
        ? `Aus der Anzeige geht nicht hervor, ob der Motor Zahnriemen oder Kette hat. Falls Zahnriemen, wäre der Wechsel fällig: ${overBy()} (${rhythm}, ${cost}).`
