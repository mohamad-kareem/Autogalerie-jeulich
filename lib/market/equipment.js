...
export function analyzeEquipment(vehicle) {
  ...
  for (const raw of rawList) {
    ...
    // Kleinanzeigen offers one tick box for both — it says sunroof, not panorama.
    const feature = /schiebedach\s*\/\s*panorama/i.test(text)
      ? { ...FEATURE_BY_ID.get("SUNROOF"), label: "Schiebe-/Panoramadach" }
      : matchFeature(text);
...
function equipmentLevel({ strong, good, known }) {
  ...
  // One sought-after extra on an otherwise bare car is not "good equipment".
  if (strong.length >= 2 || (strong.length >= 1 && good.length >= 2) || good.length >= 4) {
    return "GOOD";
  }
  return "BASIC";
}
