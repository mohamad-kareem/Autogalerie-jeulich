 ...
 *   Zahnriemen        belt or chain, and whether a change is likely due
...
import { timingBeltStatus } from "./timingBelt";
import {
  cleanText,
  currentMonths,
  median,
...
function negotiationArguments({
  ...
  if (position.total >= 4 && position.cheaper !== null) {
    ...
    if (share >= 0.4) {
      ...
      contra.push({
        ...
        text:
          position.cheaper === 0
            ? `Keines der ${position.total} Vergleichsfahrzeuge ist günstiger.`
            : `Nur ${position.cheaper} von ${position.total} Vergleichsfahrzeugen sind günstiger.`,
      });
    }
  }
  ...
  if (usage.available) {
    ...
    } else if (usage.level === "LOW" && !usage.shortTripRisk && usage.ageYears < 15) {
      // On a car this old, a low yearly figure says little — it is the age
      // that sets the price, not the kilometres.
      contra.push({
        weight: 1,
  ...
  if (belt?.available) {
    const cost = belt.costRange
      ? `ca. ${belt.costRange[0].toLocaleString("de-DE")}–${euro(belt.costRange[1])}`
      : null;
    if (belt.status === "OVERDUE") {
      pro.push({
        weight: 3,
        text: `Zahnriemenwechsel wahrscheinlich fällig${belt.engine ? ` (${belt.engine})` : ""} – ${cost} in der Werkstatt, die der Käufer trägt.`,
      });
    } else if (belt.status === "DUE_SOON") {
      pro.push({
        weight: 2,
        text: `Zahnriemenwechsel steht bald an – ${cost}.`,
      });
    } else if (belt.status === "CHECK_URGENT") {
      pro.push({
        weight: 1,
        text: `Kein Zahnriemenwechsel belegt – falls Zahnriemen, wäre er bei diesem Alter und Kilometerstand fällig (${cost}).`,
      });
    } else if (belt.status === "DONE" && belt.driveSource === "AD") {
      contra.push({ weight: 1, text: `Zahnriemen laut Anzeige gewechselt (${belt.lastChange.label}).` });
    }
  }
 ...
 */
export function inspectionChecklist({ target, usage, inspection, equipment, belt = null }) {
  const items = [];
  ...
  if (belt?.available) {
    if (["OVERDUE", "DUE_SOON", "DONE_UNVERIFIED", "DONE"].includes(belt.status)) {
      add(
        "Motor",
        belt.lastChange
          ? `Rechnung zum Zahnriemenwechsel (${belt.lastChange.label}) verlangen – ohne Nachweis gilt er als fällig`
          : "Rechnung über einen Zahnriemenwechsel verlangen – sonst Wechsel einpreisen",
        belt.label,
      );
      if (belt.drive === "WET_BELT") {
        add("Motor", "Ölwechsel-Nachweise prüfen, Motoröl auf Riemenabrieb ansehen", "Zahnriemen im Ölbad – falsches Öl zersetzt den Riemen");
      }
    } else if (belt.status === "CHAIN_WATCH") {
      add("Motor", "Kaltstart: Rasseln der Steuerkette in den ersten Sekunden?", belt.engine || "hohe Laufleistung");
    } else if (belt.status === "CHECK_URGENT" || belt.status === "CHECK") {
      add("Motor", "Klären, ob Zahnriemen oder Kette – bei Riemen Wechselnachweis verlangen", belt.label);
    }
  } else if (Number.isFinite(kmValue) && kmValue >= 100_000) {
    add("Motor", "Zahnriemen- bzw. Steuerkettenwechsel nachweisen lassen", `${km(kmValue)}`);
  }
...
export function buildInsights({ target, market, comparables, dealerCase, observations = [] }) {
  ...
  const belt = timingBeltStatus(target);
  ...
  return {
    ...
    belt,
    checklist: inspectionChecklist({ target, usage, inspection, equipment, belt }),
