...
export function computeMarketValue(comparables, target) {
  ...
  // A private seller on AutoScout24 or mobile.de is the private market too,
  // not only Kleinanzeigen — the same test the price adjustment uses.
  const isPrivate = (entry) =>
    entry.sourceLevel === "PRIVATE" || entry.sellerType === "PRIVATE";
  const retail = kept.filter((entry) => !isPrivate(entry));
  const privateAds = kept.filter(isPrivate);
  ...
  return {
    ...
    differenceToAsking:
      marketValue !== null && askingPrice !== null
        ...
        : null,
    // Measured against the market value: "52 % unter Marktwert". Against the
    // asking price a cheap car would read "111 % günstiger", which is nonsense.
    differenceToAskingPercent:
      marketValue && askingPrice !== null
        ? round(((marketValue - askingPrice) / marketValue) * 100, 1)
        : null,
...
export function computeDealerCase(target, market, options = {}) {
  ...
  // The price the ad asks, and the price actually on the table. Once the buyer
  // has spoken to the seller, the negotiated figure is what the whole
  // calculation should run on — profit and discount included.
  const listPrice = Number.isFinite(target.price) ? target.price : null;
  const negotiated = Number.isFinite(options.negotiatedPrice)
    ? Math.round(options.negotiatedPrice)
    : null;
  const asking = negotiated ?? listPrice;
  ...
  return {
    ...
    listPrice,
    negotiatedPrice: negotiated,
    // Everything below is calculated against this figure.
    askingPrice: asking,
...
export function determineVerdict({ target, market, dealerCase, confidence }) {
  if (
    ...
    !Number.isFinite(dealerCase.askingPrice ?? target.price)
  ) {
  ...
  // Use the negotiated price when there is one — that is the real decision.
  const asking = Number.isFinite(dealerCase.askingPrice)
...
export function collectWarnings({ target, market, dealerCase, threshold }) {
  const warnings = [];
  ...
  // Source availability (a portal not activated, a portal blocking us) is
  // plumbing, not advice for the buyer. It lives in the technical section,
  // which already lists every source and its state.
