...
async function fetchListingViaPage(url) {
  ...
  const merged = {};
  const used = [];
  const names = ["embedded-json", "json-ld", "anzeigentext", "seitenlabels", "meta"];
  ...
  const vehicle = createVehicle({
    ...
    listingUrl: url,
    priceType: detectPriceType(fullText),
    tuvUntil: merged.tuvUntil || detectTuv(fullText),
    serviceHistory: detectServiceHistory(fullText),
    ...
    condition: detectConditionFromText(
      `${text} ${merged.description || ""}`.slice(0, 8_000),
    ),
    sellerType: /privatanbieter|privatverkauf|privater anbieter/i.test(fullText)
      ? "PRIVATE"
      : "DEALER",
    description: merged.description || text.slice(0, 2_000),
    extractedFrom: `${via === "browser" ? "browser" : "http"}: ${used.join(" + ") || "seitentext"}`,
  });
