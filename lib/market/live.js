...
export function recalculate(result, adjust) {
  ...
  return {
    ...
    // Opening offer, target and walk-away price, from the limit on screen.
    // The stance (how hard to push) was read from the data by the server.
    negotiation:
      askingPrice === null
        ? null
        : negotiationPlan({
            limit,
            askingPrice,
            stance: result.insights?.negotiation?.stance || "NORMAL",
          }),
  };
}
