...
export async function request(
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      retryAt = Date.now() + retryAfterMs(response.headers.get("retry-after"));

      let text;
      if (response.ok && onChunk && response.body) {
        ...
        try {
          while (true) {
            ...
            // A parser may explicitly finish once its complete payload arrives.
            // Cancel the remaining body so a slow footer cannot hold the lock.
            if (chunk) { chunks.push(chunk); if (onChunk(chunk) === false) break; }
            if (done) break;
          }
      ...
      if (!retryable || attempt === retries || retryAt > Date.now()) break;
