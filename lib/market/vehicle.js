...
export function createVehicle(input = {}) {
  ...
  return {
    ...
    // Hubraum in cm³ — tells a 1.2 PureTech from a 1.6 and so belt from chain.
    displacementCcm: (() => {
      const ccm = toNumber(input.displacementCcm);
      return Number.isFinite(ccm) && ccm >= 500 && ccm <= 8_500 ? Math.round(ccm) : null;
    })(),
